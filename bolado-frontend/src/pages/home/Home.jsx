...
const GroupMatchItem = ({ match, onUpdate }) => {
    const {
        isAdmin,
        isEditing,
        setIsEditing,
                        ...
                        <div className="flex items-center gap-1.5">
                            <input
                                ...
                                onChange={(e) => {
                                    ...
                                    if (val === "" || parseInt(val) >= 0) setHomeScore(val);
                                }}
                            ...
                            <input
                                ...
                                onChange={(e) => {
                                    const val = e.target.value;
                                    if (val === "" || parseInt(val) >= 0) setAwayScore(val);
                                }}
                                className="w-8 h-8 text-center font-black bg-blue-50 border border-blue-200 rounded-lg text-blue-900 focus:outline-none focus:ring-2 focus:ring-blue-500"
                            />
...
const GroupBlock = ({ group, stageGroups, onUpdate }) => {
                ...
                <div className="lg:col-span-5 lg:border-l border-gray-100 lg:pl-10">
                    ...
                    <div className="space-y-1">
                        {groupMatches.length > 0 ? groupMatches.map((match) => (
                            <GroupMatchItem key={match.id} match={match} onUpdate={onUpdate} />
                        )) : (
...
const HomePage = () => {
    const { stageGroups, loading: loadingHome, error, refresh: refreshHome } = useHome();
    const { groups, loading: loadingGroups, refresh: refreshGroups } = useGrups();
                    ...
                    <div className="space-y-16">
                        {activePhase === 'Grupos' ? (
                            <div className="space-y-4">
                                {groups.map(group => (
                                    <GroupBlock key={group.groupName} group={group} stageGroups={stageGroups} onUpdate={handleUpdate} />
                                ))}
                            </div>
                        ) : (
